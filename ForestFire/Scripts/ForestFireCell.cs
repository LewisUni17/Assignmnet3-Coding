...
public class ForestFireCell : MonoBehaviour
{
    ...
    public Material groundMaterialRiver;
    ...
    public GameObject riverObject; // refernce to river vusual object
    ...
    // change cell state to river
    // once a cell is set to river state will never change
    // so we can check to see if the river has material has been set aleady and never alter it again to save on performance
    public void SetRiver()
    {
        if (groundMeshRenderer.sharedMaterial == groundMaterialRiver)
            return;

        // this code below wont run once the river material has been set
        ResetCell();
        cellState = State.River;
        cellFuel = 0;
        groundMeshRenderer.material = groundMaterialRiver; // sets the cell material to river
